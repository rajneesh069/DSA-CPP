...
5. maps(normal/ordered ones obviously) contain unique keys only, if we re-assign a value to the key of a map it will change the value of that key then.

### Ordered Maps: Insertion and Accessing time is O(logN)
...
### Unordered Maps : Insertion and Accessing time is O(1)
